...
**Returns**: Depends on the content type header. Parsed JSON-like data as a `dart:convert` Map or
List | Text body if content type request header is not set to `application/json`.
